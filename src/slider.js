...
export function sliderChart(root, data, DPI_WIDTH) {
  ...
  function mousedown(event) {
    const type = event.target.dataset.type //боковые arrows или окно
    const dimensions = {
    ...
    if (type === "window") {
      const startX = event.pageX //позиция окно по оси X
  ...
  yData.map(toCoords(xRatio, yRatio, DPI_HEIGHT, 0, yMin)).forEach((coords, i) => {
    ...
    const lineWidth = 2.5
    line(ctx, coords, { color }, lineWidth)
  })
